...
/**
 * GOOGLE ADS CLIENTS COMPREHENSIVE AUDIT SCRIPT (Simplified Version)
 * 
 * This script performs a complete audit of all Google Ads clients by:
 * 1. Fetching all Google Ads clients from the database
 * 2. Testing each client's API credentials and data via existing API endpoints
...
/**
 * Test Google Ads API credentials and fetch account data via existing API
 */
async function testGoogleAdsCredentials(client) {
  ...
  try {
    ...
    // 5. Test API access by calling the existing Google Ads API endpoint
    console.log(`   📋 Testing API access via existing endpoint...`);
    const apiTest = await testGoogleAdsAPI(client.id);
    results.apiCalls.accountInfo = apiTest;

    if (apiTest.error) {
      results.errors.push(`API test failed: ${apiTest.error}`);
      results.tokenStatus = 'invalid';
      return results;
    } else {
    ...
    // 6. Get campaigns data
    console.log(`   🎯 Fetching campaigns data...`);
    const campaigns = await getGoogleAdsCampaignsData(client.id);
    ...
    // 7. Get recent insights (last 30 days)
    console.log(`   📈 Fetching recent insights...`);
    const insights = await getGoogleAdsInsightsData(client.id);
...
/**
 * Test Google Ads API access using existing endpoint
 */
async function testGoogleAdsAPI(clientId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/fetch-google-ads-live-data`, {
      method: 'POST',
      headers: {
    ...
    if (!data.success) {
      return { error: data.error || 'API call returned unsuccessful response' };
    }
 ...
 */
async function getGoogleAdsCampaignsData(clientId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/fetch-google-ads-live-data`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return { error: `Campaigns API call failed: ${response.status}` };
    }

    const data = await response.json();
    
    if (!data.success) {
      return { error: data.error || 'Campaigns API call returned unsuccessful response' };
    }
...
/**
 * Get Google Ads insights data
 */
async function getGoogleAdsInsightsData(clientId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/fetch-google-ads-live-data`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return { error: `Insights API call failed: ${response.status}` };
    }

    const data = await response.json();
    
    if (!data.success) {
      return { error: data.error || 'Insights API call returned unsuccessful response' };
    }
