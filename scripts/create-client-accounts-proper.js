...
async function createClientAccountsProper() {
  console.log('🔧 CREATING CLIENT USER ACCOUNTS (PROPER)\n');
  ...
  try {
    ...
    for (const clientData of clientsToCreate) {
      ...
      try {
        ...
        if (authError) {
          console.error(`❌ Error creating auth user for ${clientData.name}:`, authError);
          continue;
        }

        console.log(`✅ Auth user created for ${clientData.name}:`, authUser.user.id);
        ...
        console.log(`✅ Profile created for ${clientData.name}`);
        ...
        console.error(`❌ Error in process for ${clientData.name}:`, error);
      }
    }
...
// Run the function
createClientAccountsProper().catch(console.error); 