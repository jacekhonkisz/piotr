#!/usr/bin/env node
/**
 * COMPREHENSIVE BACKFILL - ALL CLIENTS, MONTHLY & WEEKLY PERIODS
 * 
 * This script fetches ALL historical data from Google Ads API using the corrected
 * all_conversions metric (instead of conversions) to match Google Ads Console numbers.
 * 
 * It will:
 * 1. Fetch data for all months from January 2024 to previous month
 * 2. Fetch data for all complete weeks in the current month
 * 3. Store in google_ads_campaigns table
 * 4. Update campaign_summaries table (both monthly and weekly)
 * 5. Work for all clients automatically
 * 
 * Usage: 
 *   npx tsx scripts/backfill-all-periods-all-clients.ts --dry-run  # Check first
 *   npx tsx scripts/backfill-all-periods-all-clients.ts            # Run for real
 */
...
function getMonthsAndWeeksToBackfill(): Period[] {
  ...
  // Generate all complete months up to (but not including) current month
  while (year < currentYear || (year === currentYear && month < currentMonth)) {
    ...
    periods.push({
      ...
      label: `${year}-${String(month).padStart(2, '0')} (monthly)`
    });
    
    // Also generate weekly periods for each completed month
    const monthStart = new Date(year, month - 1, 1);
    const monthEnd = new Date(year, month, 0);
    
    let weekStart = new Date(monthStart);
    // Move to first Monday on or after month start
    ...
    while (weekStart <= monthEnd) {
      ...
      // Only include complete weeks (7 days) that don't extend beyond month
      if (weekEnd <= monthEnd && weekEnd >= weekStart) {
        const weekNum = getWeekNumber(weekStart);
        periods.push({
          type: 'weekly',
          startDate: weekStart.toISOString().split('T')[0],
          endDate: weekEnd.toISOString().split('T')[0],
          label: `Week ${weekNum} ${year} (weekly)`
        });
      }
...
async function processPeriod(
  ...
  try {
    ...
    const campaigns = await googleAdsService.getCampaignData(period.startDate, period.endDate);
    ...
    const summary = {
      client_id: client.id,
      summary_type: period.type,
      summary_date: period.startDate,
      platform: 'google',
    ...
    console.log(`     ✅ Updated campaign_summaries for ${period.label}`);
    return 'success';
...
async function main() {
  console.log('🚀 COMPREHENSIVE BACKFILL - ALL CLIENTS, MONTHLY & WEEKLY');
  console.log(`📅 Start: ${startFromMonth}`);
  ...
  console.log('📊 Will process BOTH monthly and weekly periods\n');

  const periods = getMonthsAndWeeksToBackfill();
  
  if (periods.length === 0) {
    console.log('ℹ️  No periods to backfill');
    process.exit(0);
  }

  const monthlyCount = periods.filter(p => p.type === 'monthly').length;
  const weeklyCount = periods.filter(p => p.type === 'weekly').length;

  console.log(`📊 Will backfill ${periods.length} total periods:`);
  console.log(`   - ${monthlyCount} monthly periods`);
  console.log(`   - ${weeklyCount} weekly periods\n`);
  ...
  for (const client of clients) {
    console.log('\n' + '='.repeat(80));
    console.log(`👥 CLIENT: ${client.name} (${client.google_ads_customer_id})`);
    ...
    // Process each period for this client
    for (const period of periods) {
      console.log(`\n  📅 ${period.label}: ${period.startDate} to ${period.endDate}`);
      
      const result = await processPeriod(period, client, googleAdsService, isDryRun);
      ...
      // Add a small delay between periods to avoid rate limits
      await new Promise(resolve => setTimeout(resolve, 500));
    }

    console.log(`\n  📊 Client Summary: ${clientSuccess} success, ${clientFailures} failures, ${clientSkipped} skipped`);
  }
