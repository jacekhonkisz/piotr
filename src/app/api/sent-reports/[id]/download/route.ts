...
// GET - Download PDF for a sent report
export async function GET(
  ...
  try {
    ...
    if (!downloadUrl && process.env.NODE_ENV === 'development') {
      // Return a sample PDF URL for testing
      downloadUrl = 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf';
    }

    // Generate filename for download
    const clientName = sentReport.clients?.name?.replace(/[^a-zA-Z0-9]/g, '_') || 'client';
    const reportPeriod = sentReport.report_period?.replace(/[^a-zA-Z0-9]/g, '_') || 'report';
    const filename = `${clientName}_${reportPeriod}_${new Date(sentReport.sent_at).toISOString().split('T')[0]}.pdf`;
