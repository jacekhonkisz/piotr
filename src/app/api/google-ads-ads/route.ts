...
/**
 * RMF R.40: Ad-level performance endpoint
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { clientId, adGroupId, dateStart, dateEnd } = body;

    if (!clientId || !adGroupId || !dateStart || !dateEnd) {
    ...
    // Fetch ads
    const ads = await googleAdsService.getAdPerformance(
      adGroupId,
      dateStart,
      dateEnd
    );
