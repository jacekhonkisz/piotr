...
// Main smart cache function for Google Ads monthly data
export async function getGoogleAdsSmartCacheData(clientId: string, forceRefresh: boolean = false) {
  const currentMonth = getCurrentMonthInfo();
  
  try {
    return await executeGoogleAdsSmartCacheRequest(clientId, currentMonth, forceRefresh);
  } catch (error) {
    logger.error('❌ Google Ads smart cache request failed:', error);
    throw error;
  }
}

// Main smart cache function for Google Ads weekly data
export async function getGoogleAdsSmartWeekCacheData(clientId: string, forceRefresh: boolean = false, requestedPeriodId?: string) {
  const targetWeek = requestedPeriodId ? 
    parseWeekPeriodId(requestedPeriodId) : 
    getCurrentWeekInfo();
  
  try {
    return await executeGoogleAdsSmartWeeklyCacheRequest(clientId, targetWeek, forceRefresh);
  } catch (error) {
    logger.error('❌ Google Ads weekly smart cache request failed:', error);
    throw error;
  }
}
